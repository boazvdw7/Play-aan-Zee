    ...
    <div v-if="selectedCabin">
      ...
      <form>
        <table>
          ...
          <tr>
            ...
            <td>
              <select id="type" v-model="copyCabin.type">
                <option v-for="type in CabinType" :key="type" :value="type">{{ type.toString() }}</option>
              </select>
            </td>
          </tr>
          <tr>
            ...
            <td>
              <select id="location" v-model="locationUpdater">
                <option v-for="loc of this.locations" :key="loc.id" :value="loc.id">{{ loc.name }}</option>
              </select>
          ...
          <tr>
            <td><label for="rentalRequest">Rental requests:</label></td>
            <td>Open: {{this.getNumOpenRentalRequests()}}, Confirmed: {{this.getNumConfirmedRentalRequests()}}</td>
          </tr>
          </tbody>
        </table>
      </form>
...
export default {
  ...
  methods: {
    ...
    getRentalStatus() {
      this.statusCount = { PAID: 0, CANCELLED: 0, FULFILLED: 0, REQUESTED: 0, APPROVED: 0, DECLINED: 0 };

      this.statusCount = this.copyCabin.rentals.reduce((status, rental) => {
    ...
    getNumOpenRentalRequests() {
      return this.statusCount.REQUESTED;
    },


    getNumConfirmedRentalRequests() {
      return this.statusCount.APPROVED + this.statusCount.PAID;
    }
  },
