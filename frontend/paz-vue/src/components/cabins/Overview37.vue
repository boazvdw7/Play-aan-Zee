<template>
  <div class="ml-3 mr-3">
    <h3>All Cabins Details (REST-backend):</h3>
...
export default {
  ...
  methods: {
    ...
    async onDeleteEvent(cabinToDelete) {
      await this.cabinsService.asyncDeleteById(cabinToDelete.id);
    ...
    async onSaveEvent(cabinToUpdate) {
      ...
      if (index !== -1) {
        await this.cabinsService.asyncSave(cabinToUpdate);
      }
    ...
    async onNewCabinButton() {
      let loc = await this.locationService.asyncFindAll();
      let locOne =  loc[0]

      let newCabin = Cabin.createRandomCabin(0, locOne);

      this.selectedCabin = await this.cabinsService.asyncSave(newCabin);
  ...
  watch: {
    ...
    '$route'() {
      ...
      if(this.$route.params.id) {
        this.cabinsService.asyncFindById(this.$route.params.id)
            .then((cabin) => {
